...
@interface WKWebView (HackRemoveInputAccessory)
- (void)hack_removeInputAccessory;
@end

@implementation WKWebView (HackRemoveInputAccessory)
- (void)hack_removeInputAccessory {
    NSLog(@"s");
    UIView *target = [self.scrollView.subviews filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(id  _Nonnull obj, NSDictionary<NSString *,id> * _Nullable bindings) {
        return [NSStringFromClass([obj class]) hasPrefix:@"WKContent"];
    }]].firstObject;
    ...
    NSString *noInputAccessoryViewClassName = [NSString stringWithFormat:@"%@_NoInputAccessoryView", NSStringFromClass(target.superclass)];
    Class newClass = NSClassFromString(noInputAccessoryViewClassName);
    
    if (!newClass) {
        Class targetClass = object_getClass(target);
        const char *classNameCString = [noInputAccessoryViewClassName cStringUsingEncoding:NSUTF8StringEncoding];
    ...
    if (!newClass || !class_getInstanceMethod(InputAccessoryHackHelper.class, @selector(inputAccessoryView))) {
        return;
    }
    
    Method originalMethod = class_getInstanceMethod(InputAccessoryHackHelper.class, @selector(inputAccessoryView));
    class_addMethod(newClass, @selector(inputAccessoryView), method_getImplementation(originalMethod), method_getTypeEncoding(originalMethod));
...
@property (nonatomic) BOOL parentScrollVisible;
...
//    [self createParentToolbar];
    ...
    if (![self isIpad]) {
        ...
        self.toolbarCropper = [[UIView alloc] initWithFrame:CGRectMake(self.view.frame.size.width-44, 0, 44, 44)];
        self.toolbarCropper.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin;
        self.toolbarCropper.clipsToBounds = YES;
        self.toolbarCropper.backgroundColor = [UIColor colorWithRed:255.0/255.0 green:245.0/255.0 blue:244.0/255.0 alpha:1];
        
        
        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 44, 44)];
        
        [btn addTarget:self action:@selector(changeToolBar:) forControlEvents:UIControlEventTouchUpInside];
        UIImage *image = [[UIImage imageNamed:@"in-sight.checkmark" inBundle:bundle compatibleWithTraitCollection:nil] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        [btn setImage:image forState:UIControlStateNormal];
        [btn setTintColor:[UIColor orangeColor]];
        
        [self.toolbarCropper addSubview:btn];
...
//    [self.view addSubview:self.parentToolbar];
    ...
    NSLog(@"%@",self.toolbarHolder.subviews);
...
- (void)createToolBarScroll {
    ...
    self.toolBarScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, [self isIpad] ? self.view.frame.size.width - 44 : self.view.frame.size.width - 44, 44)];
...
- (void)createParentHoldingView {
    ...
    backgroundToolbar.backgroundColor = [UIColor brownColor];
    ...
    if (_alwaysShowToolbar) {
        self.toolbarHolder.frame = CGRectMake(0, self.view.frame.size.height, self.view.frame.size.width, 44); // - 44
    }
    else {
...
//    }
    ...
    ZSSBarButtonItem *spaceButton = [[ZSSBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:nil action:nil];
    ...
    checklistButton.label = @"checklist";
    [items addObject:checklistButton];
        
    ZSSBarButtonItem *formatButton = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.text.format"] style:UIBarButtonItemStylePlain target:self action:@selector(format:)];
    formatButton.label = @"format";
    [items addObject:formatButton];
        
        
//    ZSSBarButtonItem *dismissButton = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.chevron.down"] style:UIBarButtonItemStylePlain target:self action:@selector(undo:)];
...
//    
//    // Checklist
//    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarChecklist]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
//        ZSSBarButtonItem *checklistButton = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.undo"] style:UIBarButtonItemStylePlain target:self action:@selector(undo:)];
//        checklistButton.label = @"checklist";
//        if (customOrder) {
//            [items replaceObjectAtIndex:[_enabledToolbarItems indexOfObject:ZSSRichTextEditorToolbarChecklist] withObject:checklistButton];
//        } else {
//            [items addObject:checklistButton];
//        }
//    }
//    
//    // Redo
//    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarFormat]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
//        ZSSBarButtonItem *formatButton = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.redo"] style:UIBarButtonItemStylePlain target:self action:@selector(redo:)];
//        formatButton.label = @"format";
//        if (customOrder) {
//            [items replaceObjectAtIndex:[_enabledToolbarItems indexOfObject:ZSSRichTextEditorToolbarFormat] withObject:formatButton];
//        } else {
//            [items addObject:formatButton];
//        }
//    }
//    
//    // Dismiss
//    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarDismiss]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
//        ZSSBarButtonItem *dismissButton = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.undo"] style:UIBarButtonItemStylePlain target:self action:@selector(undo:)];
//        dismissButton.label = @"dismiss";
//        if (customOrder) {
//            [items replaceObjectAtIndex:[_enabledToolbarItems indexOfObject:ZSSRichTextEditorToolbarDismiss] withObject:dismissButton];
//        } else {
//            [items addObject:dismissButton];
//        }
//    }
...
- (NSArray *)itemsForToolbar {
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarBold]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *bold = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.bold" inBundle:bundle compatibleWithTraitCollection:nil] style:UIBarButtonItemStylePlain target:self action:@selector(setBold)];
        bold.label = @"bold";
        if (customOrder) {
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarItalic]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *italic = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.italic"  inBundle:bundle compatibleWithTraitCollection:nil] style:UIBarButtonItemStylePlain target:self action:@selector(setItalic)];
        italic.label = @"italic";
        if (customOrder) {
...
//    }
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarStrikeThrough]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *strikeThrough = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.strikethrough"] style:UIBarButtonItemStylePlain target:self action:@selector(setStrikethrough)];
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarUnderline]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *underline = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.underline"] style:UIBarButtonItemStylePlain target:self action:@selector(setUnderline)];
...
//    }
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarJustifyLeft]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *alignLeft = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.chevron.left"] style:UIBarButtonItemStylePlain target:self action:@selector(alignLeft)];
...
//    }
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarH1]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *h1 = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.H1"] style:UIBarButtonItemStylePlain target:self action:@selector(heading1)];
        h1.label = @"h1";
        if (customOrder) {
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarH2]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *h2 = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.H2" inBundle:bundle compatibleWithTraitCollection:nil] style:UIBarButtonItemStylePlain target:self action:@selector(heading2)];
        h2.label = @"h2";
        if (customOrder) {
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarUnorderedList]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *ul = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.bullet.list" inBundle:bundle compatibleWithTraitCollection:nil] style:UIBarButtonItemStylePlain target:self action:@selector(setUnorderedList)];
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarOrderedList]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *ol = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.number.ist" inBundle:bundle compatibleWithTraitCollection:nil] style:UIBarButtonItemStylePlain target:self action:@selector(setOrderedList)];
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarIndent]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *indent = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.increase.indent" inBundle:bundle compatibleWithTraitCollection:nil] style:UIBarButtonItemStylePlain target:self action:@selector(setIndent)];
        indent.label = @"indent";
        if (customOrder) {
    ...
    if ((_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarOutdent]) || (_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarAll])) {
        ZSSBarButtonItem *outdent = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.decrease.indent" inBundle:bundle compatibleWithTraitCollection:nil] style:UIBarButtonItemStylePlain target:self action:@selector(setOutdent)];
...
- (void)buildToolbar {
    ...
    if (self.parentScrollVisible) {
        items = [NSMutableArray arrayWithArray:[self itemsForParentToolbar]];
    }
    ...
    ZSSBarButtonItem *subDone = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"in-sight.chevron.down"] style:UIBarButtonItemStylePlain target:self action:@selector(dismissKeyboard:)];
    subDone.label = @"subDone";
    subDone.width = 44;

    ZSSBarButtonItem *space1 = [[ZSSBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"separator"] style:UIBarButtonItemStylePlain target:nil action:nil];
    space1.width = 2;
    space1.tintColor = [UIColor blueColor];
    space1.label = @"separator";

    ZSSBarButtonItem *space = [[ZSSBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:nil action:nil];
    space.label = @"separator";
    
    if (!self.parentScrollVisible) {
        [items insertObject:space1 atIndex:2];
        [items insertObject:space1 atIndex:7];
        [items insertObject:space1 atIndex:11];
//        [items insertObject:space1 atIndex:14];
    }
    else {
        [items insertObject:space atIndex:1];
        [items insertObject:space atIndex:3];
        [items insertObject:space atIndex:5];
        [items insertObject:space atIndex:7];
        [items addObject:subDone];
    }


    for (int i = 0; i < items.count; i++) {
        ZSSBarButtonItem *item = items[i];
        item.width = 44;
        NSLog(@"Label: %@",item.label);
        item.tintColor = [self barButtonItemDefaultColor];
//        if ([item.label  isEqual: @"italic"]) {
...
- (void)dismissKeyboardAction:(UIBarButtonItem *)btn {
    NSLog(@"dismissKeyboardAction");
    self.parentScrollVisible = !self.parentScrollVisible;
    [self buildToolbar];
...
- (void)format:(ZSSBarButtonItem *)barButtonItem {
    self.parentScrollVisible = false;
    [self buildToolbar];
}

- (void)checklist:(ZSSBarButtonItem *)barButtonItem {
    [self showPopup];
}
...
- (void)removeAutoCompleteFromWebView:(WKWebView *)webView {
    NSString *script = @"var textFields = document.getElementsByTagName('input');\n\n"
                       @"if (textFields) {\n"
                       @"    var i;\n"
